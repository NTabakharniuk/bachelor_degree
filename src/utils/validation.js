 ...
 * 5. Head size is appropriate (56-70% of image height)
...
export async function validatePhoto(imageData) {
  ...
  if (detections.length === 0) {
    ...
    // Requirements: head height should be ~56–70% of the photo height (no more than 70%)
    const MIN_RATIO = 0.54; // 54%
    const MAX_RATIO = 0.7; // 70%
    ...
    if (headSizeRatio < MIN_RATIO || headSizeRatio > MAX_RATIO) {
      result.checks.headSize = {
        passed: false,
        message: `Head size ${headSizeRatio < MIN_RATIO ? 'too small' : 'too large'} (${(headSizeRatio * 100).toFixed(0)}% of photo height)`,
      };
      result.isValid = false;
    } else {
      result.checks.headSize = {
        passed: true,
        message: `Head size appropriate (${(headSizeRatio * 100).toFixed(0)}% of photo height) ✓`,
      };
    }
